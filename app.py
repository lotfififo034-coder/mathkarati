"""
مذكرتي Pro v16 — متوافق مع Python 3.11/3.12/3.13/3.14
الإصلاحات:
...
- /generate يرفض الطلبات حتى تكتمل الـ modules (يمنع 500 عند cold start)
"""
import os, sys, json, subprocess, shutil, tempfile, logging, importlib, threading, base64
...
def health():
    ...
    return jsonify({
        ...
        "modules_ready":  _preload_done.is_set(),
    }), 200

@app.route("/warmup")
def warmup():
    """
    NON-BLOCKING: يرجع الحالة الحالية فوراً.
    Frontend يستدعيه كل 3 ثواني حتى modules_ready=true.
    """
...
def generate():
    # ── حماية cold start: انتظر حتى تكتمل الـ modules ──────────────
    # يمنع 500 عشوائي إذا ضغط المستخدم Generate قبل انتهاء الـ preload
    if not _preload_done.is_set():
        ready = _preload_done.wait(timeout=30)
        if not ready:
            return jsonify({
                "error": "الخادم لا يزال يُهيئ المحركات — انتظر لحظة وأعد المحاولة"
            }), 503
...
def _gen_python(data: dict, module_name: str):
    ...
    try:
        ...
        filename  = f"mathkarati_{safe_name}{file_suffix}.pptx"

        # إرجاع base64 — يتجاوز مشكلة Render reverse-proxy مع binary streams
...
def _gen_premium(data: dict):
    try:
        ...
        latin = _re.sub(r'[^\w]', '_', name, flags=_re.ASCII).strip('_')
        safe_name = latin[:20] if latin else f"prs_{int(_time.time())}"
        filename = f"mathkarati_{safe_name}_premium.pptx"
